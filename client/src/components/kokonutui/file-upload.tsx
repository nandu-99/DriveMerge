...
export default function FileUpload({
  ...
  const simulateUpload = useCallback(
    (uploadingFile: File) => {
      ...
      // Server handles all chunking decisions based on available storage
      // Client always sends the full file - server splits if needed
      console.log('Uploading to server (server will handle storage-aware chunking if needed)');
      console.log(`   File: ${uploadingFile.name}`);
      console.log(`   Size: ${(uploadingFile.size / 1024 / 1024).toFixed(2)} MB`);
  ...
  const handleDragLeave = useCallback(
    (e: DragEvent<HTMLDivElement>) => {
      ...
      if (dragCounter.current === 0 && status !== "uploading") {
        setStatus("idle");
      }
    },
    [status]
